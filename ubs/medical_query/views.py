    ...
    def get_queryset(self):
        ...
        if self.request.GET.get("search_box", False):
            self.queryset = self.queryset.filter(
                Q(Patient_idPatient__full_name__icontains=self.request.GET["search_box"])
                | Q(current_health_history__icontains=self.request.GET["search_box"])
            )
...
@method_decorator(login_required, name="dispatch")
class ListAllQuerysHistory(ListView):

    model = Query
    http_method_names = ["get"]
    template_name = "querys/history_all.html"
    context_object_name = "object_list"
    paginate_by = 20

    def get_queryset(self):
        self.queryset = super(ListAllQuerysHistory, self).get_queryset()
        if self.request.GET.get("search_box", False):
            self.queryset = self.queryset.filter(
                Q(Patient_idPatient__full_name__icontains=self.request.GET["search_box"])
                | Q(current_health_history__icontains=self.request.GET["search_box"])
    ...
    def get_context_data(self, **kwargs):
        _super = super(ListAllQuerysHistory, self)
        context = _super.get_context_data(**kwargs)
        adjacent_pages = 3
        page_number = context["page_obj"].number
        num_pages = context["paginator"].num_pages
...
class ListAttendances(ListView):
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get("search_box", False):
            self.queryset = self.queryset.filter(
                Q(main_complaint__icontains=self.request.GET["search_box"])
                | Q(current_health_history__icontains=self.request.GET["search_box"])
            )
        return self.queryset
...
class Attendances(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({"no_edit": True, "second_form": self.second_form_class(instance=physical_exam), "medicines": medicines})
        return context
...
class ForwardingList(ListView):
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get("search_box", False):
            self.queryset = self.queryset.filter(
                Q(patient__full_name__icontains=self.request.GET["search_box"])
                | Q(medical__full_name__icontains=self.request.GET["search_box"])
            )
        return self.queryset
...
class ListMedicine(ListView):
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get("search_box", False):
            self.queryset = self.queryset.filter(
                Q(full_name__icontains=self.request.GET["search_box"]) | Q(generic_name__icontains=self.request.GET["search_box"])
            )
        return self.queryset
...
class Recipe(DetailView):
    ...
    def get(self, request, query_pk, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(current_date=datetime.now().date(), medicines=medicnes, object=Query.objects.get(id=query_pk))
        )
...
class ExamRequestPDF(DetailView):
    ...
    def get(self, request, query_pk, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(current_date=datetime.now().date(), object=Query.objects.get(id=query_pk))
        )
